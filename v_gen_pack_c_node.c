...
void verse_send_a_layer_create(VNodeID node_id, VLayerID curve_id, const char *name)
{
	uint8 *buf;
	unsigned int buffer_pos = 0, address_size = 0;
	VCMDBufHead *head;
...
#if defined V_PRINT_SEND_COMMANDS
	printf("send: a_layer_create(node_id = %u curve_id = %u name = %s );\n", node_id, curve_id, name);
#endif
	buffer_pos += vnp_raw_pack_uint32(&buf[buffer_pos], node_id);
	buffer_pos += vnp_raw_pack_uint16(&buf[buffer_pos], curve_id);
	address_size = buffer_pos;
...
void verse_send_a_layer_destroy(VNodeID node_id, VLayerID curve_id)
{
	uint8 *buf;
	unsigned int buffer_pos = 0, address_size = 0;
	VCMDBufHead *head;
	...
	buffer_pos += vnp_raw_pack_uint8(&buf[buffer_pos], 160);/* Packing the command */
#if defined V_PRINT_SEND_COMMANDS
	printf("send: a_layer_destroy(node_id = %u curve_id = %u );\n", node_id, curve_id);
#endif
	buffer_pos += vnp_raw_pack_uint32(&buf[buffer_pos], node_id);
	buffer_pos += vnp_raw_pack_uint16(&buf[buffer_pos], curve_id);
	address_size = buffer_pos;
...
{
	unsigned int buffer_pos = 0;
	void (* func_a_layer_create)(void *user_data, VNodeID node_id, VLayerID curve_id, const char *name);
	VNodeID node_id;
	VLayerID curve_id;
	char name[16];
	...
	buffer_pos += vnp_raw_unpack_string(&buf[buffer_pos], name, 16, buffer_length - buffer_pos);
#if defined V_PRINT_RECEIVE_COMMANDS
	printf("receive: a_layer_create(node_id = %u curve_id = %u name = %s ); callback = %p\n", node_id, curve_id, name, user_func);
#endif
	if(name[0] == 0)
	{
		void (* alias_a_layer_destroy)(void *user_data, VNodeID node_id, VLayerID curve_id);
		alias_a_layer_destroy = v_fs_get_alias_user_func(160);
		if(alias_a_layer_destroy != NULL)
			alias_a_layer_destroy(v_fs_get_alias_user_data(160), node_id, curve_id);
		return buffer_pos;
	}
	if(func_a_layer_create != NULL)
		func_a_layer_create(user_data, node_id, curve_id, name);

	return buffer_pos;
}

void verse_send_a_layer_subscribe(VNodeID node_id, VLayerID curve_id, VNATransferType transfer)
{
	uint8 *buf;
	unsigned int buffer_pos = 0, address_size = 0;
	VCMDBufHead *head;
...
#if defined V_PRINT_SEND_COMMANDS
	printf("send: a_layer_subscribe(node_id = %u curve_id = %u transfer = %u );\n", node_id, curve_id, transfer);
#endif
	buffer_pos += vnp_raw_pack_uint32(&buf[buffer_pos], node_id);
	buffer_pos += vnp_raw_pack_uint16(&buf[buffer_pos], curve_id);
	address_size = buffer_pos;
...
void verse_send_a_layer_unsubscribe(VNodeID node_id, VLayerID curve_id)
{
	uint8 *buf;
	unsigned int buffer_pos = 0, address_size = 0;
	VCMDBufHead *head;
	...
	buffer_pos += vnp_raw_pack_uint8(&buf[buffer_pos], 161);/* Packing the command */
#if defined V_PRINT_SEND_COMMANDS
	printf("send: a_layer_unsubscribe(node_id = %u curve_id = %u );\n", node_id, curve_id);
#endif
	buffer_pos += vnp_raw_pack_uint32(&buf[buffer_pos], node_id);
	buffer_pos += vnp_raw_pack_uint16(&buf[buffer_pos], curve_id);
	address_size = buffer_pos;
...
{
	unsigned int buffer_pos = 0;
	void (* func_a_layer_subscribe)(void *user_data, VNodeID node_id, VLayerID curve_id, VNATransferType transfer);
	VNodeID node_id;
	VLayerID curve_id;
	uint8 transfer;
...
#if defined V_PRINT_RECEIVE_COMMANDS
	printf("receive: a_layer_subscribe(node_id = %u curve_id = %u transfer = %u ); callback = %p\n", node_id, curve_id, transfer, user_func);
#endif
	if(transfer > VN_A_COMPRESSED_UNSUSTAINED)
	{
		void (* alias_a_layer_unsubscribe)(void *user_data, VNodeID node_id, VLayerID curve_id);
		alias_a_layer_unsubscribe = v_fs_get_alias_user_func(161);
		if(alias_a_layer_unsubscribe != NULL)
			alias_a_layer_unsubscribe(v_fs_get_alias_user_data(161), node_id, curve_id);
		return buffer_pos;
	}
	if(func_a_layer_subscribe != NULL)
		func_a_layer_subscribe(user_data, node_id, curve_id, (VNATransferType)transfer);
