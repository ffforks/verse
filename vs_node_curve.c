...
static void callback_send_c_curve_key_set(void *user, VNodeID node_id, VLayerID curve_id, uint32 key_id, uint8 dimensions, real64 *pre_value, uint32 *pre_pos, real64 *value, real64 pos, real64 *post_value, uint32 *post_pos)
{
	VSNodeCurve *node;
	unsigned int i, count;
	node = (VSNodeCurve *)vs_get_node(node_id, V_NT_CURVE);
	...
	{
		vs_set_subscript_session(node->head.subscribers, i);
		verse_send_c_curve_key_set(node_id, curve_id, key_id, dimensions, pre_value, pre_pos, value, pos, post_value, post_pos);
	}
	vs_reset_subscript_session();
}
...
static void callback_send_c_curve_subscribe(void *user, VNodeID node_id, VLayerID curve_id)
{
	...
	for(i = 0; i < node->curves[curve_id].length; i++)
		if(node->curves[curve_id].keys[i].pos != V_REAL64_MAX)
			verse_send_c_curve_key_set(node_id, curve_id, i, node->curves[curve_id].dimensions, node->curves[curve_id].keys[i].pre_value, node->curves[curve_id].keys[i].pre_pos, node->curves[curve_id].keys[i].value, node->curves[curve_id].keys[i].pos, node->curves[curve_id].keys[i].post_value, node->curves[curve_id].keys[i].post_pos);
}
